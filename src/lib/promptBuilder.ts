...
【炼丹系统】
炼丹时需设置火候（0-100）和时长（0-100），影响最终丹药的元素属性：
- 炼丹炉属性（furnaceElements）：配方自带的炼丹炉元素倾向
- 材料属性（herbs.elements）：材料的元素值总和
- 最终元素值 = (炼丹炉元素 × 火候系数) + (材料元素 × 时长系数)，归一化到-100~100

丹药效果由AI根据元素属性动态生成，元素值越高对应属性效果越强。
火候/时长超出配方适宜范围会降低成功率。

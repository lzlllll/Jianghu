...
【强制输出格式 — 违反将导致游戏数据错误】
你的回复必须严格分为两部分，不可遗漏任何一部分：
第一部分：叙事正文（纯叙述文字，不少于1000字）
第二部分：数据操作标记块（以 <<<OPS>>> 开头、<<<END>>> 结尾，包含所有数据变更）

重要：即使你认为当前回合没有任何数据需要修改（极少发生），也必须输出空的 <<<OPS>>>\n<<<END>>> 标记块。缺少此标记块等同于数据丢失，游戏将无法记录玩家的任何状态变化。
...
3. 【功法 ADD techniques】添加1-2个初始功法，必须包含完整字段：id、name、category、grade、rank、proficiency、proficiencyMax、basePracticeSpeed、heartCompatibility、attributes、prerequisites、skills（技能数组）、icon、description
...
8. 【新闻 ADD news.items】添加3条今日江湖新闻：官府公告、宗门布告、市井传言各一条。每条用单独一个 ADD news.items，格式：
category: 官府公告（或 宗门布告 或 市井传言）
title: 新闻标题（15字以内）
content: 新闻正文
source: 来源
...
- 集合名，用于 ADD / DELETE：inventory、log、techniques、relations、sect.tasks、sect.heritage、news.items
...
prerequisites: [{"type":"realm","value":"引气初期"}]
skills: []
icon: 🌑
description: 功法描述文本
...
6. 【强制】每轮回复末尾必须包含 <<<OPS>>>...<<<END>>> 标记块，缺一不可。如果本轮确实没有任何数据变更，输出空的 <<<OPS>>>\n<<<END>>> 即可
